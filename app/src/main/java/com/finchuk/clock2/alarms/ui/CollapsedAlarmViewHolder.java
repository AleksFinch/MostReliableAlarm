...
public class CollapsedAlarmViewHolder extends BaseAlarmViewHolder {
    ...
    @Bind(com.finchuk.clock2.R.id.countdown) AlarmCountdown mCountdown;
    @Bind(com.finchuk.clock2.R.id.recurring_days) TextView mDays; // TODO: use `new DateFormatSymbols().getShortWeekdays()` to set texts

    public CollapsedAlarmViewHolder(ViewGroup parent, OnListItemInteractionListener<Alarm> listener,
                                    AlarmController alarmController) {
        super(parent, com.finchuk.clock2.R.layout.item_collapsed_alarm, listener, alarmController);
    }
    ...
    private void bindDays(Alarm alarm) {
        ...
        String text;
        if (num == DaysOfWeek.NUM_DAYS) {
            text = getContext().getString(com.finchuk.clock2.R.string.every_day);
        } else if (num == 0) {
            text = "";
        } else {
            StringBuilder sb = new StringBuilder();
            for (int i = 0 /* Ordinal days*/; i < DaysOfWeek.NUM_DAYS; i++) {
